...
const ChatWindow = ({ onClose }) => {
  ...
  const handleSendComment = async () => {
    ...
    try {
      setSending(true);
      // POST to backend - expects auth token in headers (api instance already sets token)
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b dark:border-gray-700">
        <div>
          ...
          <div className="text-xs text-gray-500 dark:text-gray-300">System Notice</div>
        </div>
      ...
      <div className="p-3 flex-1 overflow-y-auto">
        ...
        {showRemoval && (
          ...
          >
            <p className="font-semibold text-purple-800 dark:text-purple-200">
              We’re sorry — this chat feature has been removed.
            </p>

            <p className="text-sm mt-1 text-gray-700 dark:text-gray-300">
              Our review showed some users were sharing sensitive information here. For safety,
              we discontinued the live chat.
            </p>

            <p className="text-sm mt-2 font-medium text-gray-800 dark:text-gray-100">
              Please use the Support Community page or contact our team directly.
            </p>

            <p className="text-xs mt-2 text-purple-600 dark:text-purple-300">— Thank you</p>
          </motion.div>
        )}

        {/* Compact comment box (stored in DB) */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          <textarea
            ...
            placeholder="Write your comment or collaboration idea..."
        ...
        {/* Contact section (compact display inside chat) */}
        <ContactSection />
      </div>

      {/* Disabled Input (retained to show chat is disabled) */}
